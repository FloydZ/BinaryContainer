...
//TODO NOT WORKING
/*
TEST(CmpSimple2, OffByOne_Higher_One) {
	BinaryContainerTest b1;
	BinaryContainerTest b2;
	...
	b1[63] = true;
	EXPECT_EQ(false, BinaryContainerTest::cmp_simple2(b1, b2, 0, b1.size()));
	for (int j = 0; j < b1.size() - 1; ++j) {
		uint64_t limb = BinaryContainerTest::round_down_to_limb(j);
		uint64_t mask = BinaryContainerTest::higher_mask(j) & BinaryContainerTest::lower_mask(b1.size()-1);
		EXPECT_EQ(true, BinaryContainerTest::cmp_simple2(b1, b2, limb, mask));
	}
}
//TODO NOT WORKING
TEST(CmpSimple2, Complex_One) {
	BinaryContainerTest b1;
	BinaryContainerTest b2;
	...
	for (int k_lower  = 0; k_lower < b1.size(); ++k_lower) {
		for (int k_higher = k_lower + 1; k_higher < b1.size(); ++k_higher) {
			...
			if (k_lower > 0)
				EXPECT_EQ(true, BinaryContainerTest::cmp_simple2(b1, b2, 0, k_lower));

			EXPECT_EQ(false, BinaryContainerTest::cmp_simple2(b1, b2, k_lower, k_higher));
			EXPECT_EQ(true, BinaryContainerTest::cmp_simple2(b1, b2, k_higher, b1.size()));


			b2.zero();
			EXPECT_EQ(true, BinaryContainerTest::cmp_simple2(b1, b2, k_lower, k_higher));
			...
			if (k_lower > 0)
				EXPECT_EQ(true, BinaryContainerTest::cmp_simple2(b1, b2, 0, k_lower));
			EXPECT_EQ(true, BinaryContainerTest::cmp_simple2(b1, b2, k_lower, k_higher));
			EXPECT_EQ(false, BinaryContainerTest::cmp_simple2(b1, b2, k_higher, b1.size()));
		}
	}
}
 */
...
/*TEST(Cmp, Complex_Zero) {
	...
	for (int k_lower  = 0; k_lower < b1.size(); ++k_lower) {
		for (int k_higher = k_lower + 1; k_higher < b1.size(); ++k_higher) {
			const uint64_t limb = round_down_to_limb(k_lower);
			const uint64_t mask = higher_mask(k_lower) & lower_mask(k_higher);
			...
			if (k_lower > 0)
				EXPECT_EQ(false, BinaryContainer<64>::cmp_simple2(b1, b2, 0, k_lower));

			EXPECT_EQ(true, BinaryContainer<64>::cmp_simple2(b1, b2, k_lower, k_higher));
			EXPECT_EQ(false, BinaryContainer<64>::cmp_simple2(b1, b2, k_higher, b1.size()));


			b2.zero();
			EXPECT_EQ(false, BinaryContainer<64>::cmp_simple2(b1, b2, k_lower, k_higher));
		}
	}
}*/
...
TEST(is_greater_ext2, Complex_One) {
	...
	for (int k_lower  = 0; k_lower < BinaryContainerTest2::ssize(); ++k_lower) {
		// + 65 so we always increase the counter the size of a limb
		for (int k_higher = k_lower + 64; k_higher < BinaryContainerTest2::ssize()-64; ++k_higher) {
			b2.zero();
			for (int i = 0; i < k_higher; ++i) {
				b2[i] = true;
			}
			...
			upper = BinaryContainerTest2::round_down_to_limb(BinaryContainerTest2::ssize()-1);
